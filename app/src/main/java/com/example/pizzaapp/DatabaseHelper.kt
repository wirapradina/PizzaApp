...
class DatabaseHelper(var context: Context): SQLiteOpenHelper(context,DATABASE_NAME, null, DATABASE_VERSION)
{
    ...
    fun addTransaction(){
        ...
        val username = "wirapradina@amikom.ac.id"
    ...
    fun addAccont(email:String, name:String, level:String, password:String){
    ...
    fun addMenu(menu:MenuModel){
        ...
        values.put(COLUMN_NAMA_MENU, menu.nama)
        values.put(COLUMN_PRICE_MENU, menu.harga)
        //prepare image
    ...
    fun showMenu():ArrayList<MenuModel>{
        ...
        if(cursor.moveToFirst()){
            do {
                ...
                val model = MenuModel(id = id, nama = name, harga = price, image = imageBmp)
                listModel.add(model)
